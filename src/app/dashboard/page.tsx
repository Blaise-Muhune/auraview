...
import { isOnboardingDone, markOnboardingDone } from "@/lib/ux-storage";
...
export default function Dashboard() {
  ...
  useEffect(() => {
    ...
    const run = async () => {
      ...
      const groups = groupsResult.status === 'fulfilled' ? groupsResult.value.length : 0;
      const raters = ratersResult.status === 'fulfilled' ? ratersResult.value : 0;
      if (groupsResult.status === 'fulfilled') setGroupsCount(groups);
      if (ratersResult.status === 'fulfilled') setRatersCount(raters);
      if (auraResult.status === 'fulfilled') setTotalAura(auraResult.value);

      // Returning users with activity skip the first-run flow
      if (!isOnboardingDone()) {
        if (groups > 0 || raters > 0) {
          markOnboardingDone();
        } else {
  ...
  const needsFirstAction = ratersCount === 0 && groupsCount === 0;
    ...
    <div className="min-h-screen bg-white dark:bg-gray-950">
      ...
      <main className="max-w-xl mx-auto px-5 py-10">
        <header className="mb-6">
          <h1 className="text-2xl sm:text-3xl font-semibold text-gray-900 dark:text-gray-100 mb-1">Home</h1>
          <p className="text-gray-500 dark:text-gray-400 text-sm">
            {needsFirstAction
              ? 'Get your first ratings — share a link or start a group'
              : 'Give ratings, get ratings, climb the rankings'}
          </p>
        </header>

        {needsFirstAction && (
          <div className="mb-6 p-4 rounded-xl border border-amber-200 dark:border-amber-800/40 bg-amber-50/60 dark:bg-amber-950/20">
            <p className="text-sm font-medium text-gray-900 dark:text-gray-100 mb-1">Suggested next step</p>
            <p className="text-xs text-gray-600 dark:text-gray-400 mb-3">
              Share your link with friends, or create a group for a closed friend circle.
            </p>
            <Link
        ...
        <div className="flex items-center justify-between py-4 px-5 rounded-xl bg-gray-50 dark:bg-gray-900/50 border border-gray-100 dark:border-gray-800 mb-2">
        ...
        <p className="text-xs text-gray-500 dark:text-gray-500 mb-6">
          Everyone starts at <span className="font-medium text-gray-600 dark:text-gray-400">500</span> base aura. Friend ratings (and feed) add or subtract from that.
        </p>

        <div className="space-y-3 mb-8">
          <p className="text-xs font-medium uppercase tracking-wider text-gray-400 dark:text-gray-500">Get rated</p>
          <Link href={`/profile/${user.uid}`} className="flex items-center gap-3 p-4 border-2 border-amber-500/50 dark:border-amber-400/50 rounded-xl bg-amber-50/50 dark:bg-amber-900/10 hover:bg-amber-50 dark:hover:bg-amber-900/20 transition-colors">
            ...
            <div className="flex-1 min-w-0">
              <span className="font-medium text-gray-900 dark:text-gray-100">Share your link</span>
              <p className="text-sm text-gray-500 dark:text-gray-400">Anyone can rate you — best for DMs &amp; stories</p>
            </div>
            <span className="text-amber-600 dark:text-amber-400 font-medium text-sm shrink-0">Copy link →</span>
          </Link>
          <p className="text-center text-xs text-gray-400 dark:text-gray-500">or rate each other in a closed group</p>
          <div className="grid grid-cols-2 gap-3">
            <Link href="/create-group" className="flex gap-3 p-4 border border-gray-200 dark:border-gray-800 rounded-xl hover:bg-gray-50 dark:hover:bg-gray-900/50 transition-colors">
              ...
              <div>
                ...
                <p className="text-sm text-gray-500 dark:text-gray-400 mt-0.5">Get a code to share</p>
              </div>
            </Link>
            <Link href="/join-group" className="flex gap-3 p-4 border border-gray-200 dark:border-gray-800 rounded-xl hover:bg-gray-50 dark:hover:bg-gray-900/50 transition-colors">
              ...
              <div>
                ...
                <p className="text-sm text-gray-500 dark:text-gray-400 mt-0.5">Enter a code</p>
              </div>
            </Link>
          </div>
        </div>
        ...
        <div className="pt-6 border-t border-gray-200 dark:border-gray-800">
          ...
          <div className="flex flex-wrap gap-3 mb-4">
            ...
            <Link href="/feed" className="flex-1 min-w-[120px] flex items-center justify-center gap-2 py-3 rounded-xl border border-dashed border-gray-200 dark:border-gray-700 text-gray-500 dark:text-gray-400 text-[13px] font-medium hover:bg-gray-100 dark:hover:bg-gray-800 transition-colors" title="Optional — updates from people you follow">
              Feed
              <span className="text-[10px] uppercase tracking-wide opacity-70">extra</span>
            </Link>
          </div>
          <div className="flex flex-wrap items-center gap-3">
            ...
            <span className="text-sm text-gray-500 dark:text-gray-400">
              {groupsCount} groups · {ratersCount} {ratersCount === 1 ? 'person rated you' : 'people rated you'}
            </span>
          </div>
        </div>
      </main>
