...
export default function Signup() {
  ...
  useEffect(() => {
    if (user && !loading) {
      ...
      if (pendingGroupCode) {
        // Store the group code for after onboarding
        localStorage.setItem('pendingGroupCode', pendingGroupCode);
        router.push('/onboarding');
      } else {
        router.push('/onboarding'); // Normal flow
      }
    }
  }, [user, loading, router]);
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex flex-col sm:flex-row justify-between items-center h-auto sm:h-16 py-4 sm:py-0 gap-4 sm:gap-0">
            ...
            <div className="flex gap-2 w-full sm:w-auto">
              <Link href="/login" className="flex-1 sm:flex-none px-4 py-2 rounded-lg hover:bg-gray-100 transition-colors border border-gray-200 hover:border-gray-300 text-gray-700 text-center text-sm">
                Login
              </Link>
            </div>
          </div>
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-12">
        <div className="max-w-md mx-auto">
          <div className="text-center text-gray-900 mb-6 sm:mb-8">
            <h1 className="text-3xl sm:text-4xl font-bold mb-4">Join Aura</h1>
            <p className="text-lg sm:text-xl text-gray-600">Sign up with Google to start rating your friends</p>
          </div>
