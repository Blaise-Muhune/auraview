...
const FeedIcon = () => (
  <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
    <path strokeLinecap="round" strokeLinejoin="round" d="M19 20H5a2 2 0 01-2-2V6a2 2 0 012-2h10a2 2 0 012 2v1m2 13a2 2 0 01-2-2V7m2 13a2 2 0 002-2V9a2 2 0 00-2-2h-2m-4-3H9M7 16h6M7 8h6m4 4h4m-4 0V4m0 4v4" />
  </svg>
);
const ProfileIcon = () => (
  <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
    ...
    <nav className="sticky top-0 z-10 border-b border-gray-200 dark:border-gray-800 bg-white/95 dark:bg-gray-950/95 backdrop-blur-sm">
      <div className="max-w-3xl mx-auto px-3 sm:px-4 h-12 sm:h-14 flex items-center justify-between gap-2 sm:gap-4">
        ...
        <div className="flex items-center gap-1">
          {/* Mobile: hamburger always (Feed + Rankings in menu; when logged in also Home, Groups, Profile) */}
          {(
            <button
          ...
          {/* Desktop: Feed + Rankings always; Home + Groups when logged in */}
          ...
          {user && !showAuth && (
            <div className="hidden sm:flex items-center gap-1">
              ...
              <button
                ...
                className="flex items-center justify-center w-9 h-9 px-2 py-1.5 text-gray-500 dark:text-gray-400 hover:text-red-600 dark:hover:text-red-400 rounded-lg hover:bg-red-50 dark:hover:bg-red-900/10 transition-colors"
                aria-label="Sign out"
              >
                <SignOutIcon />
              </button>
